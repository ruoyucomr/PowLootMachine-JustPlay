...
ProcessingUnit::ProcessingUnit(
        ...
        bool precomputeRefs, std::uint32_t fixedJobsPerBlock)
      ...
      bestJobsPerBlock(runner.getMinJobsPerBlock())
{
    ...
    if (fixedJobsPerBlock != 0) {
        if (!isPowerOfTwo(fixedJobsPerBlock)
                || fixedJobsPerBlock < runner.getMinJobsPerBlock()
                || fixedJobsPerBlock > runner.getMaxJobsPerBlock()) {
            throw std::runtime_error("invalid jobs per block");
        }
        bestJobsPerBlock = fixedJobsPerBlock;
    }

    /* Only tune jobs per block if we hit maximum lanes per block: */
    if (fixedJobsPerBlock == 0
            && bestLanesPerBlock == runner.getMaxLanesPerBlock()
